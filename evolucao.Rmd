...
semestre = mov.mensal.df$value[(nrow(mov.mensal.df)-6):nrow(mov.mensal.df)]

mov.mensal.df$media = mean(semestre)
...
#
# # mov.mensal.df$Index = as.character(mov.mensal.df$Index)
#
#
...
p <- plot_ly(mov.mensal.df, x = ~Index, y = ~value, name = 'Movimentação',
             type = 'scatter', mode = 'markers') %>%
        add_trace(y = ~media, name = 'Média 6m', mode = 'line')
...
# m1 <- rPlot(x = "date", y = c("psavert", "uempmed"), type = "Line", data = econ)
# m1$set(pointSize = 0, lineWidth = 1)
#
# m1$layer(y = 200, copy_layer = T, type = 'line',
#   color = list(const = 'red'),
#   tooltip = "function(item){return item._fitted}")
#
# m1
...
plot <- ev.carteira %>% ggplot(aes(x=Index, y=value, colour=variable)) +
        geom_line() + 
        labs(y="Valor", x="") +
        theme_economist() + scale_fill_economist()
...
plot <- ev.investimentos %>% ggplot(aes(Index, value, color=variable)) +
        geom_line() +
        ggtitle("Evolução dos investimentos (%)") +
